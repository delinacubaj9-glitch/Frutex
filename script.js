...
/** Customers who rate the juices */
const CUSTOMERS = [
  { id: 'kid',      emoji: '👧', name: 'Lily',       quote: 'I love sweet and fruity drinks!',          prefSweet: 9, prefTart: 2, diffs: 'easy' },
  { id: 'athlete',  emoji: '🏃', name: 'Coach Max',  quote: 'Give me something fresh and energizing!',  prefSweet: 5, prefTart: 4, diffs: 'easy' },
  { id: 'critic',   emoji: '👨‍🍳', name: 'Chef Antoine',quote: 'I demand balance and sophistication...',   prefSweet: 5, prefTart: 5, diffs: 'medium' },
  { id: 'grandma',  emoji: '👵', name: 'Grandma Rose',quote: 'Make it sweet, dear — just like my pies!', prefSweet: 8, prefTart: 2, diffs: 'easy' },
  { id: 'artist',   emoji: '🎨', name: 'Artist Mia',  quote: 'Surprise me with bold, creative flavors!',prefSweet: 6, prefTart: 6, diffs: 'medium' },
  { id: 'boss',     emoji: '👔', name: 'Mr. Sterling',quote: 'I expect perfection. Don\'t disappoint.',   prefSweet: 4, prefTart: 3, diffs: 'hard' },
  { id: 'surfer',   emoji: '🏄', name: 'Surfer Kai',  quote: 'Tropical vibes only, dude! 🤙',           prefSweet: 7, prefTart: 3, diffs: 'easy' },
  { id: 'yogi',     emoji: '🧘', name: 'Yogi Priya',  quote: 'Something pure, natural, and balanced...', prefSweet: 4, prefTart: 4, diffs: 'medium' },
];
...
/** Render a fruit's icon — SVG if available, otherwise emoji */
...
/** Frutex Natural 100% drink pairs for memory game (8 pairs = 16 cards) */
const MEMORY_DRINKS = [
  { id: 'strawberry', name: 'Strawberry', emoji: '🍓', color: '#ff6b9d', img: 'https://frutex-ks.com/wp-content/uploads/2026/03/Natural-Strawberry-200ml.webp' },
  { id: 'apple',      name: 'Apple',      emoji: '🍎', color: '#ff4757', img: 'https://frutex-ks.com/wp-content/uploads/2026/03/Natural-Apple-750ml.webp' },
  { id: 'orange',     name: 'Orange',     emoji: '🍊', color: '#ff9f43', img: 'https://frutex-ks.com/wp-content/uploads/2026/03/Natural-Orange-200ml.webp' },
  { id: 'peach',      name: 'Peach',      emoji: '🍑', color: '#ffcc80', img: 'https://frutex-ks.com/wp-content/uploads/2026/03/Natural-Peach-200ml.webp' },
  { id: 'carrot',     name: 'Carrot',     emoji: '🥕', color: '#ff8c42', img: 'https://frutex-ks.com/wp-content/uploads/2026/03/Natural-Carrot-200ml.webp' },
  { id: 'grape',      name: 'Grape',      emoji: '🍇', color: '#a855f7', img: 'https://frutex-ks.com/wp-content/uploads/2026/03/Natural-Grape-250ml.webp' },
  { id: 'blueberry',  name: 'Blueberry',  emoji: '🫐', color: '#5b7fff', img: 'https://frutex-ks.com/wp-content/uploads/2026/03/Natural-Blueberry-200ml.webp' },
  { id: 'sourcherry', name: 'Sourcherry', emoji: '🍒', color: '#dc143c', img: 'https://frutex-ks.com/wp-content/uploads/2026/03/Natural-Sourcherry-200ml.webp' },
];
...
function startMinigame() {
  // Build deck: 2 of each drink
  memoryCards = [];
  MEMORY_DRINKS.forEach(function(d) {
  ...
  memoryInterval = setInterval(function() {
    memoryTimer++;
    document.getElementById('mg-time').textContent = memoryTimer + 's';
  }, 1000);
}
...
function updateDesignPreview() {
  ...
  // Update body color (juice color)
  if (state.currentJuice) {
    body.style.background = state.currentJuice.color;
  }
...
function presentToCustomer() {
  ...
  // Pick a random customer based on player level
  var available = CUSTOMERS.filter(function(c) {
    if (c.diffs === 'hard' && state.level < 3) return false;
    if (c.diffs === 'medium' && state.level < 2) return false;
    return true;
  });
...
function getRating() {
  ...
  // --- Calculate ratings (each 0-100) ---

  // Taste: how well juice matches customer preferences
  var sweetnessMatch = 100 - Math.abs(juice.sweetness - customer.prefSweet) * 12;
  var tartnessMatch  = 100 - Math.abs(juice.tartness - customer.prefTart) * 12;
  var tasteScore = Math.max(0, Math.min(100, (sweetnessMatch + tartnessMatch) / 2));

  // Creativity: variety of fruits, number of fruits, uncommon combos
  var uniqueCats = {};
  ...
  var creativityScore = Math.min(100, juice.fruits.length * 20 + catCount * 10);

  // Appearance: bottle rarity + cap match + label
  var bottleIdx = ALL_BOTTLES.findIndex(function(b) { return b.id === design.bottle; });
  var appearanceScore = 40 + bottleIdx * 10 + 10; // base + rarity bonus

  // Overall score (weighted)
  var overallScore = Math.round(tasteScore * 0.5 + creativityScore * 0.25 + appearanceScore * 0.25);
  ...
  ratingEl.innerHTML =
    ...
    '<div class="rating-feedback">' + getFeedback(stars, overallScore) + '</div>' +
    '<div class="rating-breakdown">' +
      '<div class="rating-row">' +
        '<span>🍯 Taste</span>' +
        '<div class="rating-row-bar"><div class="rating-row-bar-fill" style="width:' + tasteScore + '%"></div></div>' +
        '<span>' + Math.round(tasteScore) + '%</span>' +
      '</div>' +
      '<div class="rating-row">' +
        '<span>🎨 Creativity</span>' +
        '<div class="rating-row-bar"><div class="rating-row-bar-fill" style="width:' + creativityScore + '%"></div></div>' +
        '<span>' + Math.round(creativityScore) + '%</span>' +
      '</div>' +
      '<div class="rating-row">' +
        '<span>✨ Appearance</span>' +
        '<div class="rating-row-bar"><div class="rating-row-bar-fill" style="width:' + appearanceScore + '%"></div></div>' +
        '<span>' + Math.round(appearanceScore) + '%</span>' +
      '</div>' +
    '</div>' +
  ...
  juice.fruits.forEach(function(id) {
    state.fruitUsage[id] = (state.fruitUsage[id] || 0) + 1;
  });
  ...
  // --- Auto-save recipe if 3+ stars ---
  if (stars >= 3) {
    saveRecipe(juice, design, stars, overallScore);
  }
...
function getFeedback(stars, score) {
  if (stars === 5) return 'Absolutely perfect! This is the best juice I\'ve ever had! 🌟';
  if (stars === 4) return 'Delicious! I really enjoyed this unique creation! 😋';
  if (stars === 3) return 'Pretty good! A solid juice with nice flavors. 👍';
  if (stars === 2) return 'It\'s okay... maybe try different fruits next time? 🤔';
  return 'Hmm, not quite my taste. Keep experimenting! 🔄';
}
...
function finishRound() {
  ...
  // Save recipe if it was good
  state.blenderFruits = [];
  state.currentJuice = null;
  state.currentDesign = { bottle: 'glass', cap: 'silver', label: 'fresh', name: '' };
